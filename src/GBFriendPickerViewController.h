...
@protocol GBFriendPickerDelegate;
@class GBFriendPickerCacheDescriptor;

/*!
 @typedef GBFriendSortOrdering enum
...
typedef enum {
    ...
    GBFriendSortByFirstName,
    /*! Sort friends by last, first, middle names. */
    GBFriendSortByLastName
} GBFriendSortOrdering;

/*!
 @typedef GBFriendDisplayOrdering enum
...
typedef enum {
    ...
    GBFriendDisplayByFirstName,
    /*! Display friends as Last First Middle. */
    GBFriendDisplayByLastName,
} GBFriendDisplayOrdering;
...
/*!
 ...
 @abstract
 The `GBFriendPickerViewController` class creates a controller object that manages
 the user interface for displaying and selecting Facebook friends.

 @discussion
 When the `GBFriendPickerViewController` view loads it creates a `UITableView` object
 ...
 The `delegate` property may be set to an object that conforms to the <GBFriendPickerDelegate>
 ...
 picker.
 */
@interface GBFriendPickerViewController : GBViewController
...
/*!
 ...
 The items in the array are <GBGraphUser> objects.

 @discussion
 You can set this this array to pre-select items in the picker. The objects in the array
 must be complete id<GBGraphUser> objects (i.e., fetched from a Graph query or from a
 ...
 */
@property (nonatomic) GBFriendSortOrdering sortOrdering;

/*!
 @abstract
 The order in which friends' names are displayed.
 */
@property (nonatomic) GBFriendDisplayOrdering displayOrdering;
...
/*!
 ...
 @abstract
 Creates a cache descriptor based on default settings of the `GBFriendPickerViewController` object.

 @discussion
 An `GBCacheDescriptor` object may be used to pre-fetch data before it is used by
 the view controller. It may also be used to configure the `GBFriendPickerViewController`
 object.
 */
...
/*!
 ...
 @abstract
 Creates a cache descriptor with additional fields and a profile ID for use with the `GBFriendPickerViewController` object.
 ...
 */
+ (GBCacheDescriptor*)cacheDescriptorWithUserID:(NSString*)userID fieldsForRequest:(NSSet*)fieldsForRequest;
...
/*!
 ...
 @abstract
 The `GBFriendPickerDelegate` protocol defines the methods used to receive event
 notifications and allow for deeper control of the <GBFriendPickerViewController>
 view.
 */
@protocol GBFriendPickerDelegate <GBViewControllerDelegate>
@optional
...
/*!
 ...
 @discussion
 The <GBFriendPickerViewController> object's `tableView` property is automatically
 ...
 */
- (void)friendPickerViewControllerDataDidChange:(GBFriendPickerViewController *)friendPicker;
 ...
 */
- (void)friendPickerViewControllerSelectionDidChange:(GBFriendPickerViewController *)friendPicker;
...
/*!
 ...
 @param user                An <GBGraphUser> object representing the friend.
 */
- (BOOL)friendPickerViewController:(GBFriendPickerViewController *)friendPicker
                 shouldIncludeUser:(id <GBGraphUser>)user;
 ...
 */
- (void)friendPickerViewController:(GBFriendPickerViewController *)friendPicker
