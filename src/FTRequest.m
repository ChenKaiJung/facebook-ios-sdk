...
static const int kGeneralErrorCode = 115;
static const int kInputErrorCode = 114;
...
+ (FTRequest *)getRequestWithParams:(NSMutableDictionary *) params
                         httpMethod:(NSString *) httpMethod
                           delegate:(id<FTRequestDelegate>) delegate
                         requestURL:(NSString *) url {
    
    FTRequest* request = [[[FTRequest alloc] init] autorelease];
...
- (id)formError:(NSInteger)code userInfo:(NSDictionary *) errorData {
    return [NSError errorWithDomain:@"funtownErrDomain" code:code userInfo:errorData];
...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error {
    ...
    if(jsonreturn == nil) {
        if (error != nil) {
            *error = [self formError:kInputErrorCode userInfo:result];
        }        
        return nil;
    }
        
    if ([jsonreturn objectForKey:@"errorCode"] == nil) {
        if (error != nil) {
            *error = [self formError:kInputErrorCode userInfo:result];
        }
        return nil;
    ...
    if([[jsonreturn objectForKey:@"errorCode"] intValue] == 100) {
        return result;
    }
    
    if (error != nil) {
        *error = [self formError:[[jsonreturn objectForKey:@"errorCode"] intValue] userInfo:result];
    }
