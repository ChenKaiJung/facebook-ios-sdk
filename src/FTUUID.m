...
@implementation FTUUID

@synthesize uuidDelegate = _uuidDelegate;

+ (id)getInstance:(id<FTUUIDDelegate>)delegate {
    
    static FTUUID *sharedFTUUID = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedFTUUID = [[self alloc] init];
    });
    sharedFTUUID.uuidDelegate = delegate;
    return sharedFTUUID;
}
...
- (void)generateUUID {
    ...
    if ([self.uuidDelegate respondsToSelector:@selector(ftDidUUIDGenerate:)]) {
        [_uuidDelegate ftDidUUIDGenerate:systemId];
    }
}
